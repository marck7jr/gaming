...
namespace Marck7JR.Gaming.Web.EpicGames.Http
{
    ...
    {
        private const string API_Country = "lc";
        private const string API_Locale = "cc";

        private const string AuthorizationCsfrUri = "https://www.epicgames.com/id/api/csrf";
        private const string AuthorizationOAuthTokenUri = "https://account-public-service-prod03.ol.epicgames.com/account/api/oauth/token";
        private const string AuthorizationSetSidUri = "https://www.epicgames.com/id/api/set-sid?sid={0}";

        private const string CatalogEndpoint = "https://catalog-public-service-prod06.ol.epicgames.com";

        private const string ExchangeGenerateUri = "https://www.epicgames.com/id/api/exchange/generate";
        ...
        private async Task<OAuthTokenResponse?> GetOAuthResponseAsync(ExchangeGenerateResponse? exchangeGenerateResponse = null, OAuthTokenResponse? oAuthTokenResponse = null)
        {
            ...
            try
            {
                ...
                var response = await _httpClient.PostAsync(AuthorizationOAuthTokenUri, formUrlEncodedContent);
        ...
        public async Task<OAuthTokenResponse?> GetOAuthTokenResponseAsync(SetSidResponse? setSidResponse)
        {
            ...
            try
            {
                _ = await _httpClient.GetAsync(string.Format(AuthorizationSetSidUri, setSidResponse.sid));
                _ = await _httpClient.GetAsync(AuthorizationCsfrUri);

                var cookie = httpClientHandler.CookieContainer.GetCookies(new Uri(AuthorizationCsfrUri))["XSRF-TOKEN"] ?? throw new ArgumentOutOfRangeException();
                _httpClient.DefaultRequestHeaders.Add(X_XSRF_TOKEN_CookieName, cookie.Value);

                var response = await _httpClient.PostAsync(ExchangeGenerateUri, null);
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    return await GetOAuthResponseAsync(exchangeCode);
                }
        ...
        public Task<OAuthTokenResponse?> GetOAuthTokenResponseAsync(OAuthTokenResponse? oAuthTokenResponse) => GetOAuthResponseAsync(oAuthTokenResponse: oAuthTokenResponse);
        ...
        public async IAsyncEnumerable<AssetsResponse> GetAssetsResponsesAsync(OAuthTokenResponse? oAuthTokenResponse)
        {
            ...
            var response = await _httpClient.GetAsync("https://launcher-public-service-prod06.ol.epicgames.com/launcher/api/public/assets/Windows?label=Live");
        ...
        public async IAsyncEnumerable<CatalogResponse> GetCatalogResponsesAsync(IEnumerable<AssetsResponse> assetsResponses)
        {
            ...
            {
                var response = await _httpClient.GetAsync($"{CatalogEndpoint}/catalog/api/shared/namespace/{assetsResponseItem.@namespace}/bulk/items?id={assetsResponseItem.catalogItemId}&includeDLCDetails=True&includeMainGameDetails=True&country={API_Country}&locale={API_Locale}");
